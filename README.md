- 👋 Hi, I’m Sivakumar Viyalan
- 🧳 I'm a Google Cloud Certified Professional Cloud (Developer, DevOps Engineer & Architect), AWS Certified {(Developer, Solutions Architect) Associate and DevOps Engineer Professional and AI Practitioner} and Microsoft Certified: GitHub Actions and HashiCorp Certified: Terraform Associate. Also I am a CNCF Kubestronaut - Kubernetes expert (CKA|CKAD|KCNA|KSCA|CKS).
- 🌱 I’m currently preparing for Kyverno and Certified Cloud Native Platform Engineering Associate (CNPA) certifications to become an CNCF Golden Kubestronaut.
- I'm interested in Agentic AI.
- Ask me about anything related to cloud (AWS, GCP, Azure), Docker, Podman, Kubernetes, OpenShift, Cilium, Istio, Python, Ansible, Terraform, Linux, Shell scripting, Git, GitHub Actions, Prometheus, GitOps, Argo, Backstage, OpenTelemetry.
- 📫 Reach me @ v.sivakumar.mca@gmail.com

<!---
sivakumarviyalan/sivakumarviyalan is a ✨ special ✨ repository because its `README.md` (this file) appears on your GitHub profile.
You can click the Preview link to take a look at your changes.
--->
